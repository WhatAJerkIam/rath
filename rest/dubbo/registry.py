...
try:
    ...
    if platform.system()=="Windows":
        os.chdir('..\\external\\kazoo-2.2.1\\')
        cmd = 'python setup.py install'
        lines = utils.run_cmd(cmd)
        os.chdir('..\\..\\tests\\')

from common import ServiceURL
...
class ZookeeperRegistry(Registry):
    ...
    def __unquote(self, origin_nodes):
        return (urllib.parse.unquote(child_node) for child_node in origin_nodes if child_node) #decode('utf8')
    ...
    def register(self, interface, **kwargs):
        ...
        if not self.__zk.exists(consumer_path + '/' + urllib.parse.quote(url, safe='')) :
